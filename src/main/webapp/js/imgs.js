...
function addCart(idd,fidd)
{
	...
	var url = ct+"/ShopServlet?key=add&id="+id+"&fid="+fid+"&str=cart";
	if(id == 0 ){
		alert("尚未登录");
	}
	else if(add.value=="加入购物车")
	{

		//ctx 由 furit_info.jsp 传入
		f.action= ct+"/ShopServlet?key=add&id="+id+"&fid="+fid+"&str=cart";
		f.submit();
		alert("加入成功")
	}
	else
...
function addStar(idd,fidd)
{
	...
	else if(add.value=="关注商品"){
		f.action= ct+"/ShopServlet?key=add&id="+id+"&fid="+fid+"&str=star";
		f.submit();
		alert("成功关注")
	}
	else
